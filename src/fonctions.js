...
const MOIS = ['january', 'febuary', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december'];
  ...
  if (thatYear == curY) {
    if (thatMonth == curM) {
      if (thatDate == curDate) {
        ...
        if ((thatDate - curDate) == 1) {
          r = `Yesterday at ${formate(date.getHours())}:${formate(m)}`;
        } else {
          curDay = (curDay == 0) ? 7 : curDay;
          thatDay = (thatDay == 0) ? 7 : thatDay;
          if (thatDay >= curDay) {
            r = `${JOUR[curDay % 7]} at ${formate(date.getHours())}:${formate(m)}`;
          } else {
            r = `${curDate}, ${MOIS[curM % 12]} at ${formate(date.getHours())}:${formate(m)}`;
          }
        }
      } else {
        r = `${curDate}, ${MOIS[curM % 12]} at ${formate(date.getHours())}:${formate(m)}`;
    ...
    r = `${formate(curDay)}/${formate(curM)}/${curY} at ${formate(date.getHours())}:${formate(m)}`;
  }
  return r;
}
