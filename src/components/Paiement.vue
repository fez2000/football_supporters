  ...
  >
    <v-card max-height="500px" style="overflow: auto">
      ...
      <v-card-text>
        ...
        <span
          v-if="project"
        >Merci d'avance pour votre contribution pour le project {{project.name}}</span>
      </v-card-text>
      <v-card-text>
        <v-row v-show="e1==1">
          <v-col>
            <v-text-field label="Montant " v-model="amount" single-line solo suffix="XAF"></v-text-field>
          </v-col>
        </v-row>

        <v-row v-show="e1==3 &&  mobile ">
          <v-col>
            <v-text-field label="Numero " v-model="phone" :single-line="false" solo prefix="+237"></v-text-field>
          </v-col>
        </v-row>
        <v-row v-show="e1 == 2">
          <v-col>
            <v-btn @click="e1=3;mobile=true">continuer par mobile</v-btn>
          </v-col>
        </v-row>
        <v-row v-show="e1==2">
          <v-col>
        ...
        <v-text-field label="Nom" v-model="first_name" v-show="e1==4 && mobile" single-line solo></v-text-field>
        <v-text-field
          label="Prenom"
          v-model="last_name "
          v-show="e1==4 &&  mobile"
          ...
          solo
        ></v-text-field>
        <v-text-field label="Email" v-model="email" v-show="e1==4 &&  mobile" single-line solo></v-text-field>
      </v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn
          color="blue darken-1"
          :disabled="(e1 == 1 && parseFloat(amount) == 'NaN' || e1 == 1 && !amount) || ((e1 == 3 && parseInt(phone) == 'NaN') ||( e1 == 3 && !phone)) "
          text
          @click="e1 ++"
          v-show="e1 == 1 || e1 == 3 "
        >Suivant</v-btn>
        <v-btn color="blue darken-1" v-show="e1 == 4" text @click="donate">Payer</v-btn>
      </v-card-actions>
    </v-card>
  </md-dialog>
</template>
