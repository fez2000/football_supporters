    ...
    <div class="wrapper">
        <parallax class="section page-header header-filter" :style="headerStyle">
            <div class="container">
                <div class="md-layout">
                    ...
                    >
                        <h1 class="title"><span v-text="nb"></span> {{$t('members.title')}}</h1>
                        <h4>
                        {{$t('members.header')}}
                        </h4>
                        <br />
                        ...
                        target="_blank"
                        ><i style="font-size: 2.1rem!important;right: 10px;position: relative;" class="mdi v-icon notranslate mdi-account-network"></i>{{$t("members.button")}}</md-button
                        >
                    </div>
                </div>
            </div>
        ...
        <div class="main main-raised">
            <div class="section">
                <div class="container">
                    <div class="md-layout">
                        ...
                        <p class="description" style="text-align:justify">{{$t("members.description1")}} <strong>{{$t("members.slogan")}}</strong></p>
            ...
            <div class="section text-center">
                ...
                <div class="team">
                    <div class="md-layout">
                        <div v-for="voter of voters" :key="voter._id" class="md-layout-item md-medium-size-33 md-small-size-100">
                            <div class="team-player">
                                <md-card class="md-card-plain">
                                    ...
                                    <small class="card-description text-muted"
                                        >{{voter.short_bio}}</small
                                    >
                                    </h4>

                                    <md-card-content>
                                        <p class="card-description">
                                            {{ (voter.bio)?(voter.bio.length > 200)?voter.bio.substr(0,150)+'...':voter.bio:''}}
                                        </p>
                                    </md-card-content>

                                    <md-card-actions v-if="voter.socials.length > 0"  style="justify-content: center;" class="text-center">
...
export default {
    ...
    head: {
        ...
        meta: [
            ...
            { property: "og:url", content: `${process.env.BASE_URL||''}members/` },
        ...
        link: [
            { rel: 'canonical', href: `${process.env.BASE_URL||''}members/`, id: 'canonical' },
    ...
    methods: {
       fetchNb(){
        ...
        getvoterstart(){
            this.$axios.get('/api/voter/findallstart')
            .then(({ data })=>{
                if(data.status){
        ...
        getvoternext(){
            this.$axios.get('/api/voter/findallnext')
            .then(({ data })=>{
                if(data.status){
