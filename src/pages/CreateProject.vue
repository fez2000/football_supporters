<template>
  <v-container :fluid="true">
          ...
          lazy-validation>
              ...
              <v-row>
                <v-col cols="12" md="4" offset-md="2" offset-lg="1"  lg="5" :order="($vuetify.breakpoint.sm || $vuetify.breakpoint.xs)?'first':'last'">
                  ...
                  <v-row align="center">
                      ...
                      <label  for="projectimg" hidden><p>{{$t("createproject.f_img")}}</p></label>
                      <input hidden name="projectimg"  id="projectimg" type="file" @input="checkFile"  accept="image/*" />
                ...
                <v-col cols="12" md="4" offset-md="2" offset-lg="1" lg="5">
                  ...
                  <v-btn :disabled="!valid || submiting || !name || cathegoriesList.length == 0 || cathegories.length == 0 " color="blue darken-1" text @click="save()">{{(submiting)?$t("createproject.f_runbutton"):$t("createproject.f_button")}}</v-btn>
                </v-col>
              </v-row>
...
export default {
  head: {
    ...
    meta: [
        ...
        { property: "og:url", content: `${process.env.BASE_URL||''}/dashboard/contribute` },
    ...
    data () {
      return {
        ...
        shortDescriptionRules: [
          ...
          v => (v && v.length <= 150) || 'Short Description must be less than 150 characters',
        ],
        descriptionRules: [
          v=> !v || true,
          v =>  !v || v.length <= 2000 || 'Descript must be less than 2000 characters',
        ],
        dialog: false,
        cathegoriesList: [],
        projectimg: null,
    ...
    methods: {
      ...
      save() {
        ...
        this.$axios.post("/api/project",data,
        {
          headers: {
      ...
      updateImgRef(docId, projectId){
        ...
        .then(({data})=>{
          ...
          this.$root.$emit('neterror', { err: err, callback: this.updateImgRef, data: { docId, projectId } });
        })
      },
      file(file) {
        this.projectimg = file;
