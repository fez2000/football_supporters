  ...
  <v-row>
    <v-col class="pa-4" offset="1" cols="11" md="5" lg="5" xl="5">
      ...
      <v-row>
        ...
        <v-btn
          :disabled="submit || !question || (!startNow&&!startDate) || !endDate "
          @click="create() "
          color="primary"
          text
        >{{(submit)?$t('pollCreate.saving'):$t('pollCreate.save')}}</v-btn>
      </v-row>
    </v-col>
  </v-row>
</template>
...
export default {
  ...
  meta: [
    ...
    {
      property: "og:url",
      content: `${process.env.BASE_URL || ""}/dashboard/poll/create`
    },
